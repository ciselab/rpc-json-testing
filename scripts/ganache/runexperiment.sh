...
# Before running this script, the docker image must be built. This can be done using 'docker build -t ganache --no-cache .' in the directory where the Dockerfile is stored.
...
docker run -it --name $3_container_$1_$2_$4 ganache $1 $2 $3 $4
