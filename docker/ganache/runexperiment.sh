...
# Before running this script, the docker image must be built. This can be done using 'docker build -t ganache:V1 --no-cache .' in the directory where the Dockerfile is stored.

# Run the container, which automatically starts the script with arguments $1 (fitnessfunction), $2 (timelimit) and $3 (server used).
docker run -it --name $3_container_$1_$2 ganache:V1.0.1 $1 $2 $3
...
mkdir exp_$3_$1_$2
docker cp $3_container_$1_$2:/ganache-cli/final_coverage_total.txt ./exp_$3_$1_$2/
docker cp $3_container_$1_$2:/ganache-cli/final_coverage_archive.txt ./exp_$3_$1_$2/
